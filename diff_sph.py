...
def init_nn_model():
    ...
    if TRAIN:
        ...
        file_dir_path = f"{file_dir_path}/saved_models/{LOAD_LIST[LOAD_NUM]}"
        # load file
        for file in os.listdir(file_dir_path):
            # if file name contain fc1
            if "fc1" in file:
                fc1.load_weights(f"{file_dir_path}/{file}", model_id=0)
            # if file name contain fc2
            if "fc2" in file:
                fc2.load_weights(f"{file_dir_path}/{file}", model_id=0)
...
def update_pressure(t: ti.int32):
    for bs, i in ti.ndrange(BATCH_SIZE, particle_num):
        # state pressure (rho / rho0)^gamma - 1
...
def update_force(t: ti.int32):
    for bs, i in ti.ndrange(BATCH_SIZE, particle_num):
        for j in range(particle_num):
            R = F_pos[bs, t, i] - F_pos[bs, t, j]
            # Pressure forces m * (pi/rhoi^2 + pj/rhoj^2) * nabla W
            F_acc[bs, t, i] += (
                -mass
            ...
            # Viscosity forces m * (velj - veli)*r / ((r + 0.01 + h^2) * rho_j) * nabla^2 W
            F_acc[bs, t, i] += (
                viscosity_scale
                * mass
